...
<?php if ($isAdminPanel): ?>
    ...
    <div class="panel-nav shadow-sm bg-white border-b border-green-100 sticky top-0 z-40">
        <div class="max-w-7xl mx-auto px-4 flex items-center overflow-x-auto py-2 scrollbar-none">
            <span class="text-xs font-bold text-emerald-800 mr-5 flex-shrink-0 bg-emerald-100 px-3 py-1.5 rounded-md border border-emerald-200">🛠️ PANEL ADMIN</span>
...
<?php elseif ($isPenulisPanel): ?>
    ...
    <div class="panel-nav shadow-sm bg-white border-b border-green-100 sticky top-0 z-40">
        <div class="max-w-7xl mx-auto px-4 flex items-center overflow-x-auto py-2 scrollbar-none">
            <span class="text-xs font-medium mr-5 flex-shrink-0 text-slate-500">Halo, <strong class="text-emerald-700"><?= htmlspecialchars($nama) ?></strong></span>
            ...
            <?php endforeach; ?>
            <div class="flex-1"></div>
            <a href="../../index.php" class="whitespace-nowrap"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9"/></svg>Lihat Publik</a>
        </div>
    </div>
...
<?php else: ?>
    <nav class="bg-white border-b border-green-100 sticky top-0 z-40 shadow-sm">
        <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
            <a href="<?= strpos($uri, 'detail.php') !== false ? 'index.php' : '#' ?>" class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-2">
                <?php if ($role === 'admin'): ?>
                    <a href="page/admin/dashboard.php" class="text-sm px-3 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700">Masuk Panel</a>
                <?php elseif ($role === 'penulis'): ?>
                    <a href="page/penulis/dashboard.php" class="text-sm px-3 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700">Masuk Panel</a>
                <?php else: ?>
                    <a href="auth/login.php" class="text-sm px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 transition shadow-md">Login Penulis</a>
                <?php endif; ?>
            </div>
        </div>
    </nav>
